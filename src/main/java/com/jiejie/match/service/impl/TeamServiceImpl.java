...
import com.jiejie.match.common.TeamStatusEnum;
import com.jiejie.match.exception.BusinessException;
import com.jiejie.match.mapper.TeamMapper;
...
import com.jiejie.match.model.vo.UserVO;
import com.jiejie.match.service.TeamService;
...
public class TeamServiceImpl extends ServiceImpl<TeamMapper, Team>
    ...
    public List<TeamUserVO> listTeams(TeamQuery teamQuery, boolean isAdmin) {
        ...
        if (teamQuery != null) {
            ...
            if (!isAdmin && !statusEnum.equals(TeamStatusEnum.PUBLIC) && !statusEnum.equals(TeamStatusEnum.SECRET)) {
    ...
    public boolean updateTeam(TeamUpdateRequest teamUpdateRequest, User loginUser) {
        ...
        Long id = teamUpdateRequest.getId();// 参数中队伍为id的参数
        if (id == null || id <= 0) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        ...
        //只有管理员或者队伍的创建者才可以修改
        if (oldTeam.getUserId()!=loginUser.getId()&&!userService.isAdmin(loginUser)){ //不是创建者也不是管理员
            throw new BusinessException(ErrorCode.NO_AUTH);
        }
        // 如果队伍状态改为加密, 必须要有代码
    ...
    public boolean quitTeam(TeamQuitRequest teamQuitRequest, User loginUser) {
        ...
        //移除关系
        return userTeamService.remove(queryWrapper); // 删除userId和teamId关系
    }
    @Override
    @Transactional(rollbackFor = Exception.class)
